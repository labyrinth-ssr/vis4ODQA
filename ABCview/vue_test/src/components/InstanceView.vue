...
export default {
  ...
  methods: {
    ...
    getTokenWidth(tokens, svg, s/* , fontSize = "1em" */) {
      ...
      let hiddenTexts = hiddenTextGroup
      ...
      hiddenTexts.remove();
      return {
        textTokenWidths: textTokenWidths,
        textTokenHeight: textTokenHeight,
      };
    },
    drawAll() {
        ...
        var q_svg = svg
          ...
          .attr(
            ...
            "translate(" +
              margin.left +
              "," +
              (margin.top + bar_chart_height + row_idx * height) +
              ")"
          );
        var q_lineLength = this.drawParagraph(
          this.q_tokens[row_idx],
          q_svg,
          cell_width,
          q_s
        );
      ...
      for (let row_idx = 0; row_idx < 10; row_idx++) {
        var ctx_svg = svg
          ...
          .attr(
            "transform",
            "translate(" +
              (margin.left + q_max_length) +
              "," +
              (margin.top + bar_chart_height + row_idx * height) +
              ")"
          );
        var ctx_lineLength = this.drawParagraph(
          this.ctx_tokens[row_idx],
          ctx_svg,
          cell_width,
          ctx_s
        );
      ...
      var retriever_bar_chart = svg
        .append("g")
        .attr("transform", "translate(" + margin.left + "," + margin.top + ")");
      var rank_bar_chart = svg
        .append("g")
        .attr(
          "transform",
          "translate(" +
      ...
      const retriever_x_subgroup = d3
        .scaleBand()
        .domain([2, 3])
        .range([0, retriever_x.bandwidth()])
        .padding([0.05]);
      const rank_x_subgroup = d3
        .scaleBand()
        .domain([2, 3])
        .range([0, rank_x.bandwidth()])
        .padding([0.05]);
      const span_x_subgroup = d3
        .scaleBand()
        .domain([2, 3])
      ...
      const subgroups = [
        retriever_x_subgroup.bandwidth(),
        rank_x_subgroup.bandwidth(),
        span_x_subgroup.bandwidth(),
      ];
      ...
      retriever_bar_chart
        ...
        .selectAll("rect")
        .data(function (d) {
          return [2, 3].map(function (key) {
            return { key: key, value: d[key], kind: d[1], token: d[0] };
          });
        })
        .join("rect")
        .attr("x", function (d) {
          if (d.kind == 0) {
            ...
            return (
              retriever_x_subgroup(d.key) +
              c * retriever_x.bandwidth() +
              0.5 * retriever_x_subgroup.bandwidth() -
              0.5 * min_bandwidth
            );
          }
        })
      ...
      rank_bar_chart
        ...
        .attr("x", function (d) {
          if (d.kind == 0) {
            ...
            return (
              rank_x_subgroup(d.key) +
              c * rank_x.bandwidth() +
              0.5 * rank_x_subgroup.bandwidth() -
              0.5 * min_bandwidth
            );
          }
        })
      ...
      span_bar_chart
        ...
        .attr("x", function (d) {
          if (d.kind == 0) {
            ...
            return (
              span_x_subgroup(d.key) +
              c * span_x.bandwidth() +
              0.5 * span_x_subgroup.bandwidth() -
              0.5 * min_bandwidth
            );
          }
        })
    ...
    drawParagraph(tokens, cell_svg, cell_width, s) {
      ...
      const tokenGap = 2;
      const rowGap =
        textTokenHeight + textTokenPadding.top + textTokenPadding.bottom + 1;

      // Add tokens
      let tokenGroup = cell_svg
        .append("g")
        .attr("id", "token-group")
        .attr(
          "transform",
          `translate(${
            GPadding.left + (Cell_SVGWidth * (1 - containerWidthFactor)) / 2
          },
                ${GPadding.top})`
        );

      let nodes = tokenGroup
        .append("g")
      ...
      nodes.each(function (_, i) {
        ...
        let curLineLength =
          curPos.x +
          textTokenWidths[i] +
          textTokenPadding.left +
          textTokenPadding.right +
          tokenGap;
        if (i + 1 < tokenNum) {
      ...
      nodes
        ...
        .attr(
          "width",
          (_, i) =>
            textTokenWidths[i] + textTokenPadding.left + textTokenPadding.right
        )
        .attr(
          "height",
          (_, i) =>
            textTokenHeight[i] + textTokenPadding.top + textTokenPadding.bottom
        )
        .attr("rx", 2)
        // .style('fill', "white")
      ...
      d3.select("#tokensvg").attr(
        "height",
        document.getElementById("token-group").getBBox().height
      );
      return lineLength;
    },
    init() {
