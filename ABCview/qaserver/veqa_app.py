...
def query_attr_tree():
    global all_layer_node_link,list_tokenPool,tokens,sentence_span_one_tree
    # top_kth=0
    if request.method=='POST':
        all_layer_node_link=[]
        post_data=request.get_json()
        ...
        for i in range(0,12):
            ...
            for inx, val in enumerate(tokens):# remember to delete static data
                nodes_list.append({'node': str(inx), 'name': val, 'saliency': layerSaliency[nodecnt] if not model=='que' else layerSaliency[nodecnt]/15})
                nodecnt+=1
        ...
        list_tokenPool.sort()
        sentence_span_one_tree = getSentenceSpan(tokens) if not model == "ctx" else [i + 1 for i in
                                                                                     getSentenceSpan(tokens)]
    else:
        pass
    return jsonify({
        ...
        'sentence_span': sentence_span_one_tree
   })
...
def query_single_attr_tree(top_kth):
    ...
    if request.method=='POST':
        ...
        ctx_tokens.extend(tokens[sep_save+1:])
        ctx_tokens.append('[SEP]')
        ret['q_node_link']=one_tree(0,'que/sample_'+str(que_id), 'relevant_q/attr_vec/vec_relevant_'+str(que_id),layer,threshold['que'],top_kth,tokenPool,ret['tree_height']['q'],'que',True,q_tokens)
        ret['tree_height']['q']=singleTreeHeight(ret['q_node_link'])
        ret['ctx_node_link']= one_tree(len(q_tokens)-1,'ctx/sample_'+str(que_id),'relevant_ctx/attr_vec/vec_relevant_'+str(que_id*20+top_kth),layer,threshold['ctx'],top_kth,tokenPool,ret['tree_height']['ctx'],'ctx',True,ctx_tokens)
        ret['tree_height']['ctx']=singleTreeHeight(ret['ctx_node_link'])
        ret['reranker_node_link']= one_tree(0,'reranker/sample_'+str(que_id), 'rank/attr_vec/vec_rank_'+str(que_id),layer,threshold['reranker'],top_kth,tokenPool,ret['tree_height']['reranker'],'reranker',True,tokens)
        ret['tree_height']['reranker']=singleTreeHeight(ret['reranker_node_link'])
        ret['reader_node_link']= one_tree(0,'reader/sample_'+str(que_id),'attr_vec/vec_end_'+str(que_id),layer,threshold['reader'],top_kth,tokenPool,ret['tree_height']['reader'],'reader',False,tokens)
